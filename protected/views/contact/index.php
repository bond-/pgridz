        ...
        <div class="accordion" id="contact-accordion">
            ...
            <div id="contact-collapse" class="accordion-body collapse in">
                <div class="accordion-inner">
                    ...
                    <label class="required" for="Contact[name]"><?php echo CHtml::encode($contact->getAttributeLabel('name'))?> <span class="required">*</span></label>
                    <?php $this->widget('bootstrap.widgets.TbTypeahead', array(
                    'model'=>$contact,'attribute'=>'name','htmlOptions'=>array('class'=>'span12','autocomplete'=>'off'),
                    'options'=>array(
                        'name'=>'typeahead',
                        'source'=>'js:function(query,callback){$.ajax("'.$this->createUrl('/contact/list').'",{data:{name:query},success:function(data){callback(JSON.parse(data));},error:function(){callback([]);}});}',
                        'matcher'=>"js:function(item) {return ~item.toLowerCase().indexOf(this.query.toLowerCase());}",
                    ),
                )); ?>
